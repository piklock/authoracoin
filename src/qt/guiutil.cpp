...
{
    // return if URI is not valid or is no authoracoin: URI
...
{
    // Convert authoracoin:// to authoracoin:
    //
    //    Cannot handle this later, because authoracoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open authoracoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Authoracoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Authoracoin.lnk
...
{
    return GetAutostartDir() / "authoracoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a authoracoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Authoracoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the authoracoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add authoracoin app to startup item list
