...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AuthoracoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("AuthoracoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("AuthoracoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("authoracoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running AuthoracoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
