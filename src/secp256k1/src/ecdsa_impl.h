...
static int secp256k1_ecdsa_sig_parse(secp256k1_ecdsa_sig_t *r, const unsigned char *sig, int size) {
    ...
    if (lens == 0) return 0;
    const unsigned char *ath = sig + 6 + lenr;
    while (lens > 0 && ath[0] == 0) {
        lens--;
        ath++;
    }
    if (lens > 32) return 0;
...
static int secp256k1_ecdsa_sig_serialize(unsigned char *sig, int *size, const secp256k1_ecdsa_sig_t *a) {
    ...
    unsigned char *rp = r, *ath = s;
    int lenR = 33, lenS = 33;
    while (lenR > 1 && rp[0] == 0 && rp[1] < 0x80) { lenR--; rp++; }
    while (lenS > 1 && ath[0] == 0 && ath[1] < 0x80) { lenS--; ath++; }
    if (*size < 6+lenS+lenR)
        return 0;
    *size = 6 + lenS + lenR;
    sig[0] = 0x30;
