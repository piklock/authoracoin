...
#if defined(NDEBUG)
#error "Authoracoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uATHR) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) {
        ...
        nSubsidy = 0.1 * COIN;
    } else if (nHeight < 22000 && nHeight >= 2000) {
        nSubsidy = 0.5 * COIN;
	} else if (nHeight < 25000 && nHeight >= 22000) {
        nSubsidy = 10 * COIN;
	} else if (nHeight < 37000 && nHeight >= 25000) {
        nSubsidy = 12 * COIN;
	} else if (nHeight < 45000 && nHeight >= 37000) {
        nSubsidy = 30 * COIN;
...
{
    RenameThread("authoracoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ATHR: We use certain sporks during IBD, so check to see if they are
